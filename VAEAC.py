...
torch.set_printoptions(precision=3, sci_mode=False)  # Keep this if you want pretty tensor prints globally
...
class VAEAC(Module):
    ...
    def batch_vlb(self, batch, mask):
        ...
        # --- Symmetric KL: average of KL(proposal||prior) and KL(prior||proposal)
        kl_qp = kl_divergence(proposal, prior).view(batch.shape[0], -1).sum(-1)
        kl_pq = kl_divergence(prior, proposal).view(batch.shape[0], -1).sum(-1)
        kl_sym = 0.5 * kl_qp + 0.5 * kl_pq
        return rec_loss - kl_sym + prior_reg
