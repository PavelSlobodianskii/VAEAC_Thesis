...
# ------------------- CHANGED: always import "model" from root, not from model_dir -------------------
...
for batch_tuple in iterator:
    ...
    batch_tuple_extended = extend_batch_tuple(batch_tuple, dataloader, batch_size)
    ...
    for groundtruth, mask, img_samples_params in zip(batch, masks, samples_params):
        save_img(groundtruth, join(args.out_dir, '%05d_groundtruth.jpg' % image_num))

        model_input_visualization = torch.tensor(groundtruth)
        model_input_visualization[mask.bool()] = 0.5
        save_img(model_input_visualization, join(args.out_dir, '%05d_input.jpg' % image_num))
        ...
        for i, sample in enumerate(img_samples):
            sample[~mask.bool()] = 0
            sample += model_input
            sample_filename = join(args.out_dir, '%05d_sample_%03d.jpg' % (image_num, i))
