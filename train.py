...
# ===== ADDED FOR DRIVE BACKUP =====
import os
drive_backup = True
drive_checkpoint_dir = '/content/drive/MyDrive/vaeac_checkpoints'
if drive_backup:
...
# === Checkpoint function with Drive backup ===
def make_checkpoint():
    ...
    if drive_backup:
        import shutil
        drive_cp = os.path.join(drive_checkpoint_dir, f'last_checkpoint_epoch_{epoch+1}.tar')
        shutil.copy(filename, drive_cp)
    # ==================================
    ...
    for i, batch in enumerate(iterator):
        ...
        if any([
            ...
            if max(validation_iwae[::-1]) <= val_iwae:
                ...
                if drive_backup:
                    import shutil
                    drive_best = os.path.join(drive_checkpoint_dir, f'best_checkpoint_epoch_{epoch+1}.tar')
                    shutil.copy(dst_filename, drive_best)
                # ==================================
