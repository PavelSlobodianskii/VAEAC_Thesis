...
def run_one_alpha(args, alpha_cfg, device="cuda"):
    ...
    NORMALIZE_EPOCHS = max(5, int(0.2 * args.epochs))
        ...
        for i, batch in enumerate(iterator):
            ...
            if use_amp:
                with torch.cuda.amp.autocast():
                    vlb = model.batch_vlb(batch, mask).mean()  # FIXED: mask, not 1-mask
            ...
            pc_cosines = [None, None, None]  # Placeholder

            with torch.no_grad():
                rec = float(model.rec_log_prob(batch, rec_params, mask).mean().item())  # FIXED: mask, not 1-mask
